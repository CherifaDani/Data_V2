...
#         df = pd.read_csv(csv_path, sep=sep, index_col=0, parse_dates=True)
#         if not df.empty:
#             df.index = pd.DatetimeIndex(df.index)
#             df.sort_index(ascending=True, inplace=True)
#             df.columns = [x.upper() for x in df.columns]
#             return df
#         else:
#             warnings.warn('Empty Dataframe: {} '.format(csv_path))
# 
#     else:
#         warnings.warn('Path error: {} '.format(csv_path))
...
def write_dict_to_csv(csv_name, dictionnary, mode='w'):
    """
...
def fill_dict_from_df(dfs, variable_name):
    ...
    return cat_name, sql_name, freq, country, min_val, max_val, mac_val, mcmv_val, mccv_val, parents, parameters, path
