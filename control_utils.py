...
def check_ncols(df):
    ...
    ncols = df.shape[1]
    if ncols < 1:
        logger.warn('The number of columns is less than 1 !..')
        c_message = 'ncols pb'
        alert_level = 3
...
def control_routine(df, df_latest, var_dict):
    ...
    df_info_dict = {'var_name': '',
                    ...
                    'gap_dates': [],
                    'fill_rate': 0,
                    'is_controlated': False,
                    'control_check': 'OK',
                    'alert_level': 0}
    # is the df empty
    [c_message, alert_level, is_empty] = check_isempty(df_latest)
    df_info_dict['empty_latest'] = is_empty
    clist.append(c_message)
    alist.append(alert_level)  # alert_level

    # checking the number of rows
    [c_message, alert_level, nrows] = check_ncols(df)
    df_info_dict['nrows'] = nrows
    clist.append(c_message)
    alist.append(alert_level)

    # checking the number of columns
    [c_message, alert_level, ncols] = check_ncols(df)
    df_info_dict['ncols'] = ncols
    clist.append(c_message)
    alist.append(alert_level)

    # Checking min and max values
    c_message, alert_level = check_min_max(df, var_dict['val_min'],
                                           var_dict['val_max'])
    clist.append(c_message)
    alist.append(alert_level)

    # mac
    c_message, alert_level, max_over_val = check_mac(df, var_dict['val_mac'])
    df_info_dict['max_mac_values'] = max_over_val
    clist.append(c_message)
    alist.append(alert_level)
    print c_message, alert_level

    # Checking maximum consecutive constant values (mccv)
    c_message, alert_level = check_mccv(df, var_dict['max_ccv'])
    clist.append(c_message)
    alist.append(alert_level)

    # Checking for gaps
    c_message, alert_level, lgaps, n_gaps = check_gaps(df, var_dict['freq'])
    df_info_dict['ngaps'] = n_gaps
    if n_gaps <= 10:
        df_info_dict['gap_dates'] = lgaps
    clist.append(c_message)
    alist.append(alert_level)

    # Checking the fill rate
    c_message, alert_level, ts_fill_rate = check_fill_rate(df, var_dict['freq'])
    df_info_dict['fill_rate'] = ts_fill_rate
    clist.append(c_message)
    alist.append(alert_level)
    ...
    if alert_level > 0:
        ...
        df_info_dict['control_check'] = c_message
    ...
    df_info_dict['is_controlated'] = True
