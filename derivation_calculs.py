...
def apply_operation(var_list, freq, operation, parameters):
    ...
    if operation == 'timeshift':
        ...
        fdf = apply_timeshift(dfs[0], freq, shift)
        ...
        fdf = apply_pctdelta(dfs[0], period=period, freq=ownfreq, inpct=True)

    elif operation == 'delta':
        period = parameters.get('period', 1)
        ownfreq = parameters.get('freq', 'B')
        fdf = apply_pctdelta(dfs[0], period=period, freq=ownfreq, inpct=False)
        ...
        acorrshort = apply_corr(dfs[0], period, inpct, lag, exponential, span=shortwindow)

        acorrlong = apply_corr(dfs[0], period, inpct, lag, exponential, span=longwindow)
        ...
        fdf = apply_cumulative_return(dfs[0], timeweight)
